...
import {
  ...
  AlertTriangle, XCircle, RefreshCw, CreditCard, TrendingUp, X, ChevronRight
} from 'lucide-react'
import { formatPeso, scoreTier, scorePercent } from '../lib/stellar'
import {
  getLoans, updateLoanStatus, updateScoreOnRepay, updateScoreOnDefault,
...
function RepayModal({ loan, wallet, onConfirm, onClose }: {
  ...
  // Score BEFORE repayment
  const scoreBefore = computeLocalScore(cache.repayment_score, 0, 0, 0)
  // Simulate what repayment_score becomes after this repayment
  const total = cache.total_loans + 1
  const repaid = cache.loans_repaid + 1
  const newRepayment = Math.min(100, Math.round((repaid / total) * 100))
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(11,31,58,.5)', backdropFilter:'blur(6px)' }}>
      <div style={{ width:480, background:'var(--surface)', borderRadius:24, padding:32, boxShadow:'0 24px 64px rgba(11,31,58,.24)', position:'relative' }}>
        ...
        <p style={{ fontSize:14, color:'var(--ink-3)', marginBottom:24 }}>Confirm your repayment for this loan. Your credit score will increase immediately.</p>
        ...
        <div style={{ background:'var(--surface-2)', borderRadius:14, padding:16, marginBottom:20, border:'1px solid var(--border-2)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, fontSize:14 }}>
            <span style={{ color:'var(--ink-3)' }}>Principal</span>
            <span style={{ fontWeight:700, color:'var(--ink)' }}>{formatPeso(loan.amount)}</span>
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, fontSize:14 }}>
            <span style={{ color:'var(--ink-3)' }}>Interest (5%)</span>
            <span style={{ fontWeight:700, color:'var(--ink)' }}>{formatPeso(loan.interest)}</span>
          </div>
          <div style={{ borderTop:'1px dashed #E2E8F0', paddingTop:10, display:'flex', justifyContent:'space-between', fontSize:17, fontWeight:800, color:'var(--ink)' }}>
            ...
            <span style={{ color:'var(--green)' }}>{formatPeso(loan.total)}</span>
          </div>
        </div>
  ...
  return (
    <div style={{ position:'fixed', top:24, right:24, zIndex:2000, width:340, background:'var(--ink)', borderRadius:18, padding:'20px 22px', boxShadow:'0 12px 40px rgba(11,31,58,.3)', display:'flex', gap:14, alignItems:'flex-start' }}>
      ...
      <div style={{ flex:1 }}>
        ...
        <div style={{ fontSize:13, color:'rgba(255,255,255,.65)' }}>Your credit score increased by <strong style={{ color:'#4ADE80' }}>+{scoreDiff} pts</strong></div>
...
// ── Main Component ─────────────────────────────────────────
const TABS: LoanStatus[] = ['Pending','Approved','Disbursed','Repaid','Defaulted']
const TAB_LABELS: Record<LoanStatus, string> = { Pending:'Pending', Approved:'Approved', Disbursed:'Active', Repaid:'Repaid', Defaulted:'Defaulted', Rejected:'Rejected' }
...
export default function LoanTracking({ wallet }: { wallet: WalletHook }) {
  ...
  function refresh() {
    const all = getLoans()
    if (wallet.publicKey) {
      setLoans(all.filter(l => l.wallet === wallet.publicKey || l.wallet === 'unknown'))
    } else {
      setLoans(all)
    }
  }
  ...
  useEffect(() => {
    // Auto-default overdue disbursed loans and penalise score
    const all = getLoans()
    let changed = false
    all.forEach(l => {
      if (l.status === 'Disbursed' && l.dueAt && new Date(l.dueAt) < new Date()) {
        updateLoanStatus(l.id, 'Defaulted')
        if (l.wallet) updateScoreOnDefault(l.wallet)
        changed = true
      }
    })
    if (changed) {
      // Show a score-drop banner if it's the current user's loans
      const myDefaults = all.filter(l =>
        l.status === 'Disbursed' && l.dueAt && new Date(l.dueAt) < new Date() &&
        (l.wallet === wallet.publicKey || l.wallet === 'unknown')
      )
      if (myDefaults.length > 0) {
        setSuccessInfo(null) // clear any old success
        setDefaultedInfo({ count: myDefaults.length })
      }
    }
    refresh()
  }, [wallet.publicKey])

  // Auto-switch tab to where loans exist
  useEffect(() => {
    if (loans.length > 0 && !loans.find(l => l.status === activeTab)) {
      const firstWithLoans = TABS.find(t => loans.some(l => l.status === t))
      if (firstWithLoans) setActiveTab(firstWithLoans)
    }
  }, [loans])

  function handleAdvance(id: string, next: LoanStatus) {
    updateLoanStatus(id, next)
    refresh()
    if (next === 'Disbursed') setActiveTab('Disbursed')
  }

  function handleRepayConfirm() {
    ...
    // Update loan status
    updateLoanStatus(repayingLoan.id, 'Repaid')
    // Update score cache
    const updated = updateScoreOnRepay(w)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--surface-2)', fontFamily:"'Outfit',system-ui,sans-serif", padding:32 }}>
      ...
      {successInfo && (
        <RepaySuccessBanner
          newScore={successInfo.newScore}
          scoreDiff={successInfo.diff}
          onDismiss={() => setSuccessInfo(null)}
        />
      )}

      {/* Default penalty banner */}
      {defaultedInfo && (
        <div style={{ position:'fixed', top:24, right:24, zIndex:2000, width:360, background:'#DC2626', borderRadius:18, padding:'20px 22px', boxShadow:'0 12px 40px rgba(220,38,38,.35)', display:'flex', gap:14, alignItems:'flex-start' }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:13, color:'rgba(255,255,255,.8)', lineHeight:1.5 }}>
              {defaultedInfo.count} loan{defaultedInfo.count > 1 ? 's have' : ' has'} passed the due date. Your credit score decreased due to the default penalty.
            </div>
          </div>
      ...
      <div style={{ maxWidth:800, margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
          <div>
            ...
            <p style={{ color:'var(--ink-3)' }}>{loans.length} loan{loans.length !== 1 ? 's' : ''} total</p>
          </div>
          <button onClick={refresh} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'9px 16px', borderRadius:10, border:'1.5px solid var(--border)', background:'var(--surface)', fontSize:13, fontWeight:700, color:'var(--ink-3)', cursor:'pointer' }}>
        ...
        {visibleTabs.length > 0 && (
          <div style={{ display:'flex', gap:4, marginBottom:20, borderBottom:'2px solid #E2E8F0' }}>
            {visibleTabs.map(t => (
              <button key={t} onClick={() => setActiveTab(t)} style={{ padding:'10px 18px', fontSize:14, fontWeight:700, border:'none', background:'none', cursor:'pointer', color: activeTab===t ? 'var(--ink)' : '#94A3B8', borderBottom: activeTab===t ? '2px solid #0B1F3A' : '2px solid transparent', marginBottom:-2, display:'inline-flex', alignItems:'center', gap:6 }}>
                {TAB_LABELS[t]}
                ...
                <div key={loan.id} style={{ background:'var(--surface)', borderRadius:18, border:`1.5px solid ${isActive ? '#BBF7D0' : '#E2E8F0'}`, overflow:'hidden', boxShadow: isActive ? '0 4px 20px rgba(22,163,74,.1)' : '0 1px 3px rgba(15,23,42,.04)' }}>
                  ...
                  {isActive && (
                    <div style={{ background:'var(--green-tint)', borderBottom:'1px solid #BBF7D0', padding:'10px 24px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, fontWeight:700, color:'#15803D' }}>
                        <Zap size={14} strokeWidth={2} /> Active Loan — repayment due {loan.dueAt ? formatDate(loan.dueAt) : '—'}
                      </div>
                  ...
                  <div style={{ padding:24 }}>
                    ...
                    {isActive && loan.dueAt && (
                      <div style={{ marginTop:16 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'var(--ink-3)', marginBottom:6 }}>
                          ...
                          <span style={{ fontWeight:700, color: daysUntil(loan.dueAt) <= 3 ? '#DC2626' : '#16A34A' }}>
                            {daysUntil(loan.dueAt)} days remaining
                          </span>
                        </div>
                    ...
                    <div style={{ marginTop:16, paddingTop:16, borderTop:'1px solid #F1F5F9', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                      ...
                      {/* REPAY BUTTON — prominent, only for active loans */}
                      {isActive && (
                        <button onClick={() => setRepayingLoan(loan)} style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'11px 22px', borderRadius:11, fontSize:14, fontWeight:700, color:'#fff', background:'var(--green)', border:'none', cursor:'pointer', boxShadow:'0 3px 12px rgba(22,163,74,.28)' }}>
                          <CheckCircle size={16} strokeWidth={2} /> Repay Loan — {formatPeso(loan.total)}
                        </button>
                      )}

                      {/* DEMO FLOW: Pending → Approved */}
                      {isPending && (
                        <>
                          <button onClick={() => handleAdvance(loan.id, 'Approved')} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'10px 18px', borderRadius:10, fontSize:13, fontWeight:700, color:'#3B82F6', background:'#EFF6FF', border:'1px solid #BFDBFE', cursor:'pointer' }}>
                            <ChevronRight size={14} strokeWidth={2} /> Simulate Lender Approval
                          </button>
                          <span style={{ fontSize:12, color:'var(--ink-4)' }}>Waiting for lender to approve</span>
                        </>
                      )}

                      {/* DEMO FLOW: Approved → Disbursed */}
                      {isApproved && (
                        <>
                          <button onClick={() => handleAdvance(loan.id, 'Disbursed')} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'10px 18px', borderRadius:10, fontSize:13, fontWeight:700, color:'var(--green)', background:'var(--green-tint)', border:'1px solid #BBF7D0', cursor:'pointer' }}>
                            <Zap size={14} strokeWidth={2} /> Simulate Disbursement
                          </button>
                          <span style={{ fontSize:12, color:'var(--ink-4)' }}>Lender sends XLM to your wallet</span>
                        </>
                      )}

                      {/* Repaid state message */}
                      {loan.status === 'Repaid' && (
                        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:'var(--green)', fontWeight:700 }}>
                          <CheckCircle size={15} strokeWidth={2} /> Loan repaid — your credit score has been updated
                        </div>
                      )}

                      {loan.status === 'Defaulted' && (
                        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:'#DC2626', fontWeight:700 }}>
                          <AlertTriangle size={15} strokeWidth={2} /> This loan defaulted — your vouchers were slashed
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {loans.length > 0 && (
          <div style={{ textAlign:'center', marginTop:24 }}>
            <button onClick={() => nav('/apply')} style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'11px 22px', borderRadius:12, fontSize:14, fontWeight:700, color:'var(--green)', background:'var(--green-tint)', border:'1px solid #BBF7D0', cursor:'pointer' }}>
