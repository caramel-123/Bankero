  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(11,31,58,.5)', backdropFilter: 'blur(6px)', padding: 20 }}>
      <div style={{ width: 560, maxHeight: '85vh', overflowY: 'auto', background: 'var(--surface)', borderRadius: 24, padding: 32, boxShadow: '0 24px 64px rgba(11,31,58,.24)', position: 'relative' }}>
        ...
        <div style={{ marginTop: 20, borderRadius: 'var(--r-lg)', border: '1.5px solid #BBF7D0', overflow: 'hidden', background: 'var(--green-tint)' }}>
          <button
            onClick={() => setOverallOpen(v => !v)}
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '18px 20px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left' }}
          >
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--ink)', flex: 1 }}>Putting it together — your overall 300–850 score</span>
            <ChevronDown size={16} strokeWidth={2} color="var(--ink-4)" style={{ flexShrink: 0, transform: overallOpen ? 'rotate(180deg)' : 'none', transition: 'transform 200ms' }} />
          </button>
